...
def update_file(filename, title, desc, main_content):
    ...
    content = re.sub(
        ...
        f'<meta property="og:url" content="https://www.mealos.com/{filename}">',
        content,
        flags=re.DOTALL
    )
    content = re.sub(
        r'<meta property="twitter:url" content="https://www\.mealos\.com/">',
        f'<meta property="twitter:url" content="https://www.mealos.com/{filename}">',
        content,
        flags=re.DOTALL
    )
    content = re.sub(
        r'<link rel="canonical" href="https://www\.mealos\.com/">',
        f'<link rel="canonical" href="https://www.mealos.com/{filename}">',
        content,
        flags=re.DOTALL
    )
        ...
        <section class="relative min-h-[70vh] flex items-center justify-center pt-32 pb-20 px-4 md:px-12 bg-background overflow-hidden border-b-3 border-foreground">
            ...
            <div class="max-w-4xl mx-auto w-full text-center relative z-10 reveal">
                ...
                <p class="font-sans font-medium text-lg max-w-2xl mx-auto leading-snug mb-10 opacity-80">
                    Skello est le spécialiste du planning. Meal OS est le spécialiste du restaurant. Pourquoi multiplier les abonnements quand une seule plateforme peut gérer vos plannings, centraliser vos fiches techniques, automatiser l'HACCP et garantir votre communication interne ?
                </p>
                <div class="flex flex-col sm:flex-row gap-4 justify-center">
                    <a href="#demo" class="btn-retro">Tester Meal OS (50 places)</a>
                </div>
            </div>
        </section>
...
koust_main = """
        <section class="relative min-h-[70vh] flex items-center justify-center pt-32 pb-20 px-4 md:px-12 bg-[#F4F1EB] overflow-hidden border-b-3 border-foreground">
            <div class="max-w-4xl mx-auto w-full text-center relative z-10 reveal">
                ...
                <p class="font-sans font-medium text-lg max-w-2xl mx-auto leading-snug mb-10 opacity-80">
                    Koust est puissant pour gérer vos stocks très précisément. Mais il faut un ingénieur pour l'utiliser en cuisine. Meal OS propose une approche "terrain", simple, pour contrôler son Food Cost tout en intégrant RH et HACCP que Koust n'a pas !
                </p>
                <div class="flex flex-col sm:flex-row gap-4 justify-center">
...
combo_main = """
        <section class="relative min-h-[70vh] flex items-center justify-center pt-32 pb-20 px-4 md:px-12 bg-white overflow-hidden border-b-3 border-foreground">
            ...
            <div class="max-w-4xl mx-auto w-full text-center relative z-10 reveal">
                ...
                <p class="font-sans font-medium text-lg max-w-2xl mx-auto leading-snug mb-10 opacity-80">
                    Combo connecte vos équipes au planning. C'est déjà bien. Mais Meal OS ajoute à cela le suivi opérationnel, l'hygiène HACCP, et le suivi financier (Food Cost). Ne payez pas une plateforme juste pour des plannings.
                </p>
                <div class="flex flex-col sm:flex-row gap-4 justify-center">
                    <a href="#demo" class="btn-retro">Tester Meal OS (50 places)</a>
                </div>
...
common_footer = """
        <section id="demo" class="py-24 px-4 md:px-12 bg-foreground border-t-3 border-foreground">
            <div class="max-w-5xl mx-auto card-retro bg-white md:p-0 flex flex-col md:flex-row overflow-hidden reveal">
                <div class="md:w-5/12 bg-accent text-background p-10 md:p-12 relative border-b-3 md:border-b-0 md:border-r-3 border-foreground border-dashed flex flex-col justify-center">
                    <div class="absolute inset-0 bg-black opacity-5" style="background-image: radial-gradient(circle, #000 2px, transparent 2px); background-size: 10px 10px;"></div>
                    <div class="relative z-10">
                        ...
                        <p class="font-sans font-medium opacity-90 mb-8">
                            Rejoignez la bêta de Meal OS. L'outil ultime.
                        </p>
                    </div>
                </div>
...
update_file(
    'meal-os-vs-skello.html', 
    'Skello vs Meal OS : La Meilleure Alternative (2026)', 
    "Découvrez pourquoi Meal OS est la meilleure alternative à Skello. Gérez vos plannings, vos normes HACCP et vos équipes avec un seul outil tout-en-un.", 
    skello_main + common_footer
)

update_file(
    'meal-os-vs-koust.html', 
    'Koust vs Meal OS : Simplifiez vos fiches recettes (2026)', 
    "L'alternative terrain à Koust. Contrôlez vos rendements, votre food cost et formez votre équipe avec Meal OS, plus simple et tout-en-un.", 
    koust_main + common_footer
)

update_file(
    'meal-os-vs-combo.html', 
    'Combo (ex Snapshift) vs Meal OS : Le comparatif (2026)', 
    "L'alternative idéale à Combo. Gérez les plannings et les pointages, mais intégrez aussi HACCP, pertes et opérations grâce à Meal OS.", 
